...
def get_db_connection():
    ...
    conn = psycopg2.connect(
        host="localhost", database="tbd_medsos", user="medsos", password="123456"
    )
    return conn
...
def fetch_socmed_data():
    ...
    cur = conn.cursor()
    cur.execute(
        f"SELECT timestamp, count, unique_count FROM socmed WHERE social_media = '{social_media}' and timestamp >= '{start}' and timestamp <= '{end}';"
    )
    data = cur.fetchall()
    cur.close()
    conn.close()
